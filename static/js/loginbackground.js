	...
	for (var y = 0, y2 = imagedata.height; y < y2; y += 2) {
		for (var x = 0, x2 = imagedata.width; x < x2; x += 2) {
			if (imagedata.data[(x * 4 + y * 4 * imagedata.width) + 3] > 128) {
				...
				vertex.speed = Math.random() / 200 + 0.015; // Math.random() / 200 + 0.015;
...
var init = function() {
	...
	renderer.setClearColor(0x232E40); // 0xFFFFFF
...
var render = function(a) {
	...
	for (var i = 0, j = particles.geometry.vertices.length; i < j; i++) {
		...
		particle.x += (particle.destination.x - particle.x) * .15; // particle.speed
		particle.y += (particle.destination.y - particle.y) * .15; // particle.speed
		particle.z += (particle.destination.z - particle.z) * .15; // particle.speed
	}
	console.log(a-previousTime>100)
